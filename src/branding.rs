...
// Just the big center glyph from each card (`OTF_LIGHT_PATHS[4]`/
// `TTF_LIGHT_PATHS[4]` above), on its own — no card outline, no corner
// label, for the small format badge next to a font's name in list/grid
// rows (`ui::draw_format_badge`).
